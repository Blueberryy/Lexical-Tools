...
function Fade(ent)
	...
	if (WireLib) then
		Wire_TriggerOutput(ent, "FadeActive", 1);
	end
	...
	ent._fade.unfrozen = phys:IsMovable();
	ent._fade.velocity = phys:GetVelcotiy();
...
function SetupDoor(owner, ent, data)
	...
	ent._fade.numpadUp = numpad.OnUp(  ply, stuff.key, "Fading Doors onUp", ent);
	ent._fade.numpadDn = numpad.OnDown(ply, stuff.key, "Fading Doors onDown", ent);
...
numpad.Register("Fading Doors onUp",   function(_, ent) InputOn(ent);  end);
numpad.Register("Fading Doors onDown", function(_, ent) InputOff(ent); end);
