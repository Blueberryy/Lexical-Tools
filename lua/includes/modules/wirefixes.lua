...
function WireLib.AddSpecialInputs(ent, names, types, descs)
	...
	return WireLib.AdjustSpeicalInputs(ent, names, types, descs)
end
...
function WireLib.AddSpecialOutputs(ent, names, types, descs)
	...
	return WireLib.AdjustSpeicalOutputs(ent, names, types, descs)
end


function Wire_RemoveInputs(ent, names)
	if not HasPorts(ent) then return end
	WireLib.AdjustSpeicalInputs(ent, GetExistingPorts(ent.Inputs, CreateKeys(names)))
end

function Wire_RemoveOutputs(ent, names)
	if not HasPorts(ent) then return end
	WireLib.AdjustSpeicalOutputs(ent, GetExistingPorts(ent.Outputs, CreateKeys(names)))
end
